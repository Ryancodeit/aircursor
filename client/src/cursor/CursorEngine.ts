...
export class CursorEngine {
  ...
  public setPosition(x: number, y: number) {
    this.x = Math.min(Math.max(10, x), this.viewportWidth - 10);
    this.y = Math.min(Math.max(10, y), this.viewportHeight - 10);
    this.updateElementTransform();
  }
  ...
  public start() {
    ...
    const renderLoop = () => {
      ...
      if (this.pendingDx !== 0 || this.pendingDy !== 0) {
        const stepDx = Math.sign(this.pendingDx) * Math.min(Math.abs(this.pendingDx), this.config.maxVelocity);
        const stepDy = Math.sign(this.pendingDy) * Math.min(Math.abs(this.pendingDy), this.config.maxVelocity);
  ...
  private updateElementTransform() {
    if (this.cursorElement) {
      this.cursorElement.style.transform = `translate3d(${this.x}px, ${this.y}px, 0)`;
    }
  }
}
